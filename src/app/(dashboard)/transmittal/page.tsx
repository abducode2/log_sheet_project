...
export default function TransmittalPage() {
  ...
  const FIELDS = useMemo<FieldDef[]>(() => [
    ...
    { key:'transmittal_no',  label: p.transNoField,          type:'text',   required:true, prefixDynamic:{ fromField:'element', map:TR_PREFIX } },
  ...
  const COL_HEADERS = useMemo(() => [
    { key:'transmittal_no',  label: p.transNoField,          w: undefined },
  ...
  const fetchCounts = useCallback(async () => {
    const { data } = await supabase.from('document_transmittals').select('id, parent_id, element')
    if (!data) return
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    let q = supabase.from('document_transmittals').select('*')
  ...
  async function getNextNo(): Promise<number> {
    const { data } = await supabase.from('document_transmittals').select('no')
  ...
  async function doSave(id: string, createRevision: boolean) {
    ...
    if (createRevision && editSt === 'C') {
      ...
      await supabase.from('document_transmittals').insert({
        ...
        transmittal_no: row.transmittal_no, description: row.description,
  ...
  return (
    <>
                  ...
                  group.rows.map((row, ri) => {
                    ...
                    return (
                      <tr key={row.id}
                        ...
                        <td className="cell-mono cell-blue">{row.transmittal_no}</td>
                        ...
                        <td style={{textAlign:'center', padding:'6px 8px'}}>
                          {uploadingId === row.id ? (
                            ...
                            <div style={{display:'flex',flexDirection:'column',gap:3,alignItems:'center'}}>
                              <button
                                onClick={() => setViewingPdf({ url: getCloudinaryViewerUrl(row.pdf_url!), name: row.transmittal_no ?? String(row.no), directUrl: row.pdf_url! })}
                              ...
                              {(isEditor || isAdmin) && (
                                <button onClick={async () => {
                                  ...
                                  const { error } = await supabase.from('document_transmittals').update({ pdf_url: null }).eq('id', row.id)
      ...
      {showAdd && isEditor && (
        <AddRecordModal table="document_transmittals" title={p.addBtn}
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ width:420 }}>
            ...
            <div style={{ background:'var(--bg3)', border:'1px solid #da363333', borderRadius:'var(--radius)', padding:16, marginBottom:20 }}>
              <div style={{ fontFamily:'var(--mono)', fontSize:12, color:'var(--blue)', marginBottom:4 }}>{confirmDel.transmittal_no}</div>
