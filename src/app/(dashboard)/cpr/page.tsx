...
const STATUS_COLORS: Record<string,string> = {
  ALL:'#8b949e', A:'#3fb950', B:'#58a6ff', C:'#d29922', D:'#f85149', P:'#8b949e'
}

const MIX_DESIGNS = [
  '20 MPA OPC','25 MPA OPC','30 MPA OPC','35 MPA OPC','40 MPA OPC',
  '20 MPA SRC','35 MPA SRC','C20','C25','C30','C35','C40',
]
...
export default function CprPage() {
  ...
  const STATUS_TABS = useMemo(() => [
    { key:'ALL', label: d.elements.ALL, color: STATUS_COLORS.ALL },
    { key:'A',   label: d.statusLabels.A, color: STATUS_COLORS.A },
    { key:'B',   label: d.statusLabels.B, color: STATUS_COLORS.B },
    { key:'C',   label: d.statusLabels.C, color: STATUS_COLORS.C },
    { key:'D',   label: d.statusLabels.D, color: STATUS_COLORS.D },
    { key:'P',   label: d.statusLabels.P, color: STATUS_COLORS.P },
  ], [d])

  const STATUS_LABELS: Record<string,string> = useMemo(() => ({ ...d.statusLabels }), [d])

  const FIELDS = useMemo<FieldDef[]>(() => [
    { key:'cpr_no',        label: p.cprNoField,         type:'text',   required:true, prefixStatic:'J500-RWF-CPR-' },
    { key:'description',   label: p.descField,           type:'text',   required:true },
    { key:'pour_date',     label: d.fields.pourDate,     type:'date',   required:true },
    { key:'rev',           label: d.fields.rev,          type:'number' },
    { key:'volume_m3',     label: d.cols.volumeM3,       type:'number' },
    { key:'mix_design',    label: d.cols.mixDesign,      type:'select', options: MIX_DESIGNS },
    { key:'ac_co',         label: d.fields.status,       type:'select', defaultValue:'P', options: STATUS_OPTS },
  ...
  const COL_HEADERS = useMemo(() => [
    { key:'cpr_no',      label: p.cprNoField,     w: undefined },
    { key:'description', label: p.descCol,         w: undefined },
    { key:'rev',         label: d.cols.rev,        w: 55 },
    { key:'pour_date',   label: d.cols.pourDate,   w: undefined },
    { key:'ac_co',       label: d.cols.status,     w: 130 },
  ], [d, p])
  ...
  const fetchCounts = useCallback(async () => {
    const { data } = await supabase.from('concrete_pour_requests').select('id, parent_id, ac_co')
    if (!data) return
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    let q = supabase.from('concrete_pour_requests').select('*')
  ...
  async function getNextNo(): Promise<number> {
    const { data } = await supabase.from('concrete_pour_requests').select('no')
  ...
  function onEditDateChange(dt: string) {
    ...
    const vt = calcVtime(row?.pour_date ?? null, dt)
  ...
  async function doSave(id: string, createRevision: boolean) {
    ...
    if (createRevision && editSt === 'C') {
      ...
      await supabase.from('concrete_pour_requests').insert({
        ...
        cpr_no: row.cpr_no ?? '', description: row.description,
        rev: nextRev, ac_co: 'P', pour_date: today(),
        approval_date: null, v_time: null, remarks: row.remarks,
        parent_id: id, is_archived: false, revision_count: nextRev,
      })
    } else {
      await supabase.from('concrete_pour_requests').update({
  ...
  return (
    <>
        ...
        <div className="table-wrap">
          <div className="table-header">
            <span className="table-title">
              {STATUS_TABS.find(e=>e.key===activeEl)?.label}
            </span>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th style={{width:120}}>{d.cols.approvalDate}</th>
                  <th style={{width:70}}>{d.cols.vtime}</th>
                  <th style={{width:70}}>{d.cols.volumeM3}</th>
                  <th style={{width:100}}>{d.cols.mixDesign}</th>
                  ...
                  group.rows.map((row, ri) => {
                    ...
                    return (
                      <tr key={row.id}
                        ...
                        <td className="cell-mono cell-blue">{row.cpr_no ?? row.request_no}</td>
                        ...
                        <td className="cell-mono" style={{ color:'var(--orange)', fontWeight:600 }}>
                        ...
                        <td style={{textAlign:'center', padding:'6px 8px'}}>
                          {uploadingId === row.id ? (
                            ...
                            <div style={{display:'flex',flexDirection:'column',gap:3,alignItems:'center'}}>
                              <button
                                onClick={() => setViewingPdf({ url: getCloudinaryViewerUrl(row.pdf_url!), name: row.cpr_no ?? String(row.no), directUrl: row.pdf_url! })}
                              ...
                              {(isEditor || isAdmin) && (
                                <button onClick={async () => {
                                  ...
                                  const { error } = await supabase.from('concrete_pour_requests').update({ pdf_url: null }).eq('id', row.id)
      ...
      {showAdd && (
        <AddRecordModal table="concrete_pour_requests" title={p.addBtn}
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ width:420 }}>
            ...
            <div style={{ background:'var(--bg3)', border:'1px solid #da363333', borderRadius:'var(--radius)', padding:16, marginBottom:20 }}>
              <div style={{ fontFamily:'var(--mono)', fontSize:12, color:'var(--blue)', marginBottom:4 }}>{confirmDel.cpr_no ?? confirmDel.request_no}</div>
              <div style={{ fontSize:13, marginBottom:8 }}>{confirmDel.description}</div>
              <div style={{ fontSize:11, color:'var(--text3)' }}>REV.{confirmDel.rev} · {confirmDel.ac_co}</div>
            </div>
            <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
